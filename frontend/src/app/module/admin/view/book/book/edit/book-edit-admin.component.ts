...
export class BookEditAdminComponent implements OnInit {
    ...
    public prepareEdit() {
        this.item.editionDate = this.service.format(this.item.editionDate);
    }
 ...
 public edit(): void {
    ...
    private validateCopiesSerialNumber(){
   ...
   public async openCreateAuthor(author: string) {
        const isPermistted = await this.roleService.isPermitted('Author', 'edit');
        if (isPermistted) {
