...
@Component({
  selector: 'app-book-create-admin',
  templateUrl: './book-create-admin.component.html'
})
export class BookCreateAdminComponent  implements OnInit {
   ...
   private _validBookAvailable = true;
	...
	constructor(private service: BookAdminService , private categoryService: CategoryAdminService, private copyService: CopyAdminService, private authorService: AuthorAdminService, @Inject(PLATFORM_ID) private platformId? ) {
...
/*    public save(): void {
    ...
    public save(): void {
        ...
        if (this.errorMessages.length === 0) {
            ...
            if (imageFile) {
                this.service.saveWithImage(this.item, imageFile).subscribe(
                    (savedBook) => {
                        this.items.push(savedBook);
            ...
            this.messageService.add({ severity: 'error', summary: 'Errors', detail: 'Please correct the errors on the form' });
        }
    }
