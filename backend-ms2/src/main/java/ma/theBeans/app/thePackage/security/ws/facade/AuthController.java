package ma.theBeans.app.thePackage.security.ws.facade;
...
import ma.theBeans.app.thePackage.security.dao.facade.core.RoleDao;
import ma.theBeans.app.thePackage.security.payload.request.ActivationRequest;
...
import ma.theBeans.app.thePackage.security.ws.converter.UserConverter;

import ma.theBeans.app.thePackage.security.jwt.JwtUtils;
import ma.theBeans.app.thePackage.security.payload.request.LoginRequest;
import ma.theBeans.app.thePackage.security.payload.response.JwtResponse;

import ma.theBeans.app.thePackage.transverse.emailling.EmailRequest;
import ma.theBeans.app.thePackage.transverse.emailling.EmailService;

import ma.theBeans.app.thePackage.security.common.AuthoritiesConstants;

import ma.theBeans.app.thePackage.security.service.facade.LoginAttemptService;
...
public class AuthController {
    ...
    private ResponseEntity<Map<String, String>> register(UserDto userDto, String roleUsers) {
        ...
        userDto.setEnabled(true); //change it to false khalil
        ...
        emailRequest.setBody("Discover Your Next Favorite Book at Our Online Library !! Your activation code is : " + userDto.getLinkValidationCode());
        userConverter.setRoleUsers(true);

        User user = userConverter.toItem(userDto);

        Role role = roleService.findByAuthority(roleUsers);
        if (role == null) {
            role = new Role();
            role.setAuthority(roleUsers);
            role.setCreatedAt(LocalDateTime.now());
            // persist the new role in DB
            role = roleService.create(role);
        }

        RoleUser roleUser = new RoleUser();
        roleUser.setRole(role); // Now 'role' is not transient
