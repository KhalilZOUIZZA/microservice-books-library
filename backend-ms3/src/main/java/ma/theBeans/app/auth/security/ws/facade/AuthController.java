package ma.theBeans.app.auth.security.ws.facade;
...
import ma.theBeans.app.auth.security.payload.request.ActivationRequest;
import ma.theBeans.app.auth.security.service.facade.UserService;
import ma.theBeans.app.auth.security.ws.dto.ClientDto;
import ma.theBeans.app.auth.security.ws.dto.ForgetPasswordRequest;
import ma.theBeans.app.auth.security.ws.dto.UserDto;
import ma.theBeans.app.auth.security.ws.converter.UserConverter;

import ma.theBeans.app.auth.security.jwt.JwtUtils;
import ma.theBeans.app.auth.security.payload.request.LoginRequest;
import ma.theBeans.app.auth.security.payload.response.JwtResponse;

import ma.theBeans.app.auth.transverse.emailling.EmailRequest;
import ma.theBeans.app.auth.transverse.emailling.EmailService;

import ma.theBeans.app.auth.security.common.AuthoritiesConstants;

import ma.theBeans.app.auth.security.service.facade.LoginAttemptService;
...
public class AuthController {
    ...
    public ResponseEntity<Map<String, String>> registerClient(@RequestBody UserDto userDto) {
        return register(userDto, AuthoritiesConstants.CLIENT);
    }
    @GetMapping("user/{username}")
    public ClientDto findUser(@PathVariable String username) {
    ...
    private ResponseEntity<Map<String, String>> register(UserDto userDto, String roleUsers) {
        ...
        emailRequest.setBody("Welcome to Your Books Library !! Your activation code is : " + userDto.getLinkValidationCode());
    ...
    public ResponseEntity<Map<String, String>> sendResetCode(@RequestParam String email) {
        ...
        String resetCode = userService.generateCode(6);
        user.setLinkValidationCode(resetCode);

        userService.update(user);

        EmailRequest emailRequest = new EmailRequest();
        emailRequest.setFrom("khalilzoiza7@gmail.com");
        emailRequest.setTo(email);
        emailRequest.setBcc("khalilzoiza7@gmail.com");
        emailRequest.setCc("khalilzoiza7@gmail.com");
        emailRequest.setSubject("Password Reset Code");
        emailRequest.setBody("Your Password reset code is: " + resetCode);
        emailService.sendSimpleMessage(emailRequest);
